...
import { bind } from 'min-dash/lib/fn';
...
export default class ZoomScroll {
  ...
  constructor(eventBus, canvas, config) {
    ...
    eventBus.on('canvas.init', (e) => {
      this._init(newEnabled);
    });
  }
  ...
  zoom(delta, position) {
    ...
    if (abs(this._cachedDelta) > DELTA_THRESHOLD) {
  ...
  _zoom(delta, position, stepSize) {
    ...
    var newLinearZoomLevel = round(currentLinearZoomLevel / stepSize) * stepSize;
    ...
    var newLogZoomLevel = pow(10, newLinearZoomLevel);
