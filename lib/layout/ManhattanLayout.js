...
export function repairConnection(source, target, start, end, waypoints, hints) {
  ...
  var preferredLayouts = hints.preferredLayouts,
      straight = preferredLayouts.indexOf('straight') !== -1,
      repairedWaypoints;
  ...
  if (!repairedWaypoints) {
    ...
    if (hints.connectionEnd) {
      repairedWaypoints = repairConnectionSide(target, source, end, waypoints.slice().reverse());
      repairedWaypoints = repairedWaypoints && repairedWaypoints.reverse();
    } else
    if (hints.connectionStart) {
      repairedWaypoints = repairConnectionSide(source, target, start, waypoints);
    } else
 ...
 */
function repairConnectionSide(moved, other, newDocking, points) {
  ...
  if (slicedPoints !== newPoints) {
    return repairConnectionSide(moved, other, newDocking, slicedPoints);
  }
