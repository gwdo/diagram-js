...
export default class AutoResize extends CommandInterceptor {
  ...
  constructor(eventBus, elementRegistry, modeling, rules) {
    super(eventBus);
    ...
    this.postExecuted([ 'elements.move' ], function(event) {
      ...
      var context = event.context,
          elements = unionBy('id', ...values(context.closure.topLevel)),
          hints = context.hints;
   ...
   */
  _getOptimalBounds(elements, target) {
...
function boundsChanged(newBounds, oldBounds) {
  ...
  return [ 'x', 'y', 'width', 'height' ].some(function(attr) {
    return newBounds[attr] !== oldBounds[attr];
  });
