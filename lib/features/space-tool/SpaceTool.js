...
import { getDirection as getSpaceDirection } from './SpaceUtil';
...
export default class SpaceTool {
  ...
  constructor(eventBus, dragging, canvas, modeling, rules, toolManager) {
   ...
   */
  activateSelection(event, autoActivate, reactivate) {
   ...
   */
  makeSpace(movingShapes, resizingShapes, delta, direction) {
  ...
  initializeMakeSpace(event, context) {
    ...
    assign(context, adjustments, {
      axis: axis,
      direction: getSpaceDirection(axis, offset)
    });
   ...
   * @return {Object}
   */
  calculateAdjustments(elements, axis, offset, spacePos) {
