...
export default class Overlays {
  ...
  get(search) {
    ...
    if (search.element) {
      ...
      if (container) {
        return search.type ? filter(container.overlays, pattern({ type: search.type })) : container.overlays.slice();
      } else {
        return [];
      }
    } else
    if (search.type) {
      return filter(this._overlays, pattern({ type: search.type }));
    } else {
  ...
  _createOverlayContainer(element) {
  ...
  _getOverlayContainer(element, raw) {
  ...
  _updateOverlayVisibilty(overlay, viewbox) {
    ...
    if (show) {
      if (
        ...
        (isDefined(minZoom) && maxZoom < viewbox.scale)
      ) {
        visible = false;
      }
  ...
  _updateOverlayScale(overlay, viewbox) {
  ...
  _updateOverlaysVisibilty(viewbox) {
