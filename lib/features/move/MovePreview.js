...
export default class MovePreview {
  ...
  constructor(eventBus, elementRegistry, canvas, styles, previewSupport) {
    ...
    function getAllDraggedElements(shapes) {
      ...
      return union(allShapes, ...allConnections);
    }
    ...
    eventBus.on('shape.move.start', LOW_PRIORITY, function(event) {
      ...
      if (!allDraggedElements) {
        ...
        allDraggedElements = union(allDraggedElements, getAllDraggedElements(dragShapes));
      }
...
function haveDifferentParents(elements) {
  ...
  var groupedByParents = groupBy(elements, function(e) { return e.parent && e.parent.id; });

  var parentsCount = reduce(groupedByParents, (r) => r + 1, 0);
...
function union(...elementCollections) {
  return unionBy('id', ...elementCollections);
}