...
export function reattachPoint(bounds, newBounds, point) {
  ...
  return {
    x: round((newBounds.x + newBounds.width / 2)) - floor(((bounds.x + bounds.width / 2) - point.x) / sx),
    y: round((newBounds.y + newBounds.height / 2)) - floor(((bounds.y + bounds.height / 2) - point.y) / sy)
  };
}
