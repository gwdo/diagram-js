...
export default class PasteHandler {
  ...
  constructor(eventBus, canvas, selection, elementFactory, modeling, rules) {
  ...
  preExecute(context) {
    ...
    forEach(tree, (elements, depthStr) => {
  ...
  postExecute(context) {
    ...
    forEach(labels, (labelDescriptor) => {
  ...
  _createConnection(element, parent, parentCenter, tree) {
    var modeling = this._modeling,
        rules = this._rules;
  ...
  _createShape(element, parent, position, isAttach, hints) {
    var modeling = this._modeling,
...
/**
 * A clone method that converts enumerable properties in an
 * object tree into a plain, serializable object tree.
 *
...
function clone(obj) {
  ...
  if (isObject(obj)) {
    return reduce(obj, function(target, val, key) {
