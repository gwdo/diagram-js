...
/**
 * A feature implementing copy and paste of
 * diagram elements.
 */
export default class CopyPaste {

  constructor(eventBus, modeling, elementFactory, rules, clipboard, canvas) {
  ...
  copy(selectedElements) {
    ...
    bbox = this._bbox = boundsCenter(getBBox(tree.allShapes));
      ...
      forEach(elements, function(element) {
        ...
        if (element.labelTarget) {
          ...
          if (labelTarget.waypoints) {
            delta = pointDelta(element, labelTarget.waypoints[0]);
          } else {
            delta = pointDelta(element, labelTarget);
          }
        ...
        if (element.priority === 3) {
          ...
          forEach(element.waypoints, (waypoint) => {
            var waypointDelta = pointDelta(waypoint, bbox);

            delta.push(waypointDelta);
          });
        } else {
          delta = pointDelta(element, bbox);
        }
  ...
  paste(context) {
    ...
    newTree = reduce(tree, (pasteTree, elements, depthStr) => {
  ...
  _computeDelta(elements, element) {
    var bbox = this._bbox,
        delta = {};

    // set label's relative position to their label target
    if (element.labelTarget) {
      return pointDelta(element, element.labelTarget);
    }
  ...
  createTree(elements) {
    ...
    includedElements = map(includedElements, (data) => {
